...
use berrywiki_sync::{SyncError, SyncOutcome, SyncedStore};
...
fn nonleaf_move_is_a_single_atomic_commit() {
    ...
    assert_eq!(
        commit_count(&sb, &sb.ours),
        before + 1,
        "the whole cascade is one commit"
    );
    ...
    let files = files_in(&sb, &sb.ours, "HEAD");
    assert!(
        files.iter().any(|f| f == "Parent--Child.md"),
        "descendant's new prefixed name: {files:?}"
    );
    assert!(
        files.iter().any(|f| f == "Child.md"),
        "descendant's old name (deleted): {files:?}"
    );
    assert!(
        files.iter().any(|f| f == "_Sidebar.md"),
...
fn attachment_commit_does_not_touch_the_sidebar() {
    ...
    let saved = w
        .add_attachment("d-1", "diagram.png", b"\x89PNG\r\n\x1a\n")
        .unwrap();
    assert!(saved.commit.is_some());
    ...
    let files = files_in(&sb, &sb.ours, "HEAD");
    assert!(
        files.iter().any(|f| f.contains("diagram.png")),
        "asset committed: {files:?}"
    );
    assert!(
        !files.iter().any(|f| f == "_Sidebar.md"),
        "no sidebar change: {files:?}"
    );
}
...
fn external_edit_is_checkpointed_as_its_own_commit() {
    ...
    assert!(
        saved.checkpoint.is_some(),
        "the external edit was checkpointed first"
    );
    ...
    assert!(
        !files_in(&sb, &sb.ours, "HEAD")
            .iter()
            .any(|f| f == "Loose-Note.md"),
        "not folded into the create commit"
    );
}
...
fn external_edit_to_the_same_page_is_checkpointed_not_clobbered() {
    ...
    let err = w
        .update_page("t-1", "# Target\n\nmy replacement\n")
        .unwrap_err();
    assert!(
        matches!(err, SyncError::Store(_)),
        "the foreign edit trips StaleWrite: {err}"
    );

    // The foreign edit is safe in history and still on disk — never clobbered.
    assert_eq!(
        subject(&sb, &sb.ours, "HEAD"),
        "Record changes made outside BerryWiki"
    );
    assert!(fs::read_to_string(&path)
        .unwrap()
        .contains("foreign addition"));
}
...
fn an_in_progress_merge_is_refused() {
    ...
    assert!(
        !sb.git(&sb.ours, &["merge", "--no-edit", "origin/main"])
            .success
    );
...
fn a_detached_head_is_refused_even_when_clean() {
    ...
    sb.git(&sb.ours, &["checkout", &head])
        .expect_success("detach"); // clean, detached
    let mut w = synced(&sb.ours);

    let err = w.create_page(page("Y", "y-1", None)).unwrap_err();
    assert!(
        matches!(err, SyncError::DetachedHead),
        "detached HEAD refused: {err}"
    );
}
...
fn sync_integrates_a_remote_only_change_by_fast_forward() {
    ...
    let mut w = synced(&sb.ours);
    assert_eq!(
        w.sync().unwrap().outcome,
        SyncOutcome::Integrated { fetched: 1 }
    );
    assert!(
        sb.ours.join("New-Remote-Page.md").exists(),
        "remote file now local"
    );
    assert!(
        w.list_pages()
            .iter()
            .any(|p| p.path == "New-Remote-Page.md"),
        "graph reloaded"
    );
    assert!(w.status().unwrap().is_clean());
}
...
fn sync_publishes_local_commits() {
    ...
    assert_eq!(
        w.sync().unwrap().outcome,
        SyncOutcome::Published { pushed: 1 }
    );
...
fn two_parties_round_trip_through_the_remote() {
    ...
    assert_eq!(
        a.sync().unwrap().outcome,
        SyncOutcome::Published { pushed: 1 }
    );

    let mut b = synced(&sb.theirs);
    assert_eq!(
        b.sync().unwrap().outcome,
        SyncOutcome::Integrated { fetched: 1 }
    );
...
fn diverged_history_is_handed_off_without_any_merge() {
    ...
    assert_eq!(
        handoff.upstream.0, their_tip,
        "handoff.upstream is the fetched remote tip"
    );
    ...
    assert!(
        !sb.ours.join(".git/MERGE_HEAD").exists(),
...
fn a_push_race_maps_to_pushraced_then_diverged_on_resync() {
    ...
    assert_eq!(
        sb.head(&sb.remote),
        remote_tip,
        "remote not force-updated by the rejected push"
    );
