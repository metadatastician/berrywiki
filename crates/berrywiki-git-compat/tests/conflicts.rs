...
fn remote_change_is_detected_before_push() {
    ...
    assert_eq!(
        sb.behind_by(&sb.ours),
        1,
        "remote change visible after fetch"
    );
}
...
fn non_fast_forward_push_is_rejected_and_local_commit_survives() {
    ...
    assert_eq!(
        sb.head(&sb.ours),
        our_head,
        "local commit untouched by rejection"
    );
...
fn non_overlapping_changes_merge_cleanly() {
    ...
    assert!(fs::read_to_string(sb.ours.join("Research.md"))
        .unwrap()
        .contains("theirs"));
    assert!(fs::read_to_string(sb.ours.join("Teaching.md"))
        .unwrap()
        .contains("ours"));
    sb.git(&sb.ours, &["push", "origin", "main"])
        .expect_success("push after merge");
}
...
fn same_page_conflict_preserves_both_versions_and_aborts_safely() {
    ...
    assert!(status
        .stdout
        .lines()
        .any(|l| l.starts_with("UU") && l.contains(PAGE)));

    // Aborting restores our version exactly; our commit is still HEAD.
    sb.git(&sb.ours, &["merge", "--abort"])
...
fn sidebar_conflict_behaves_like_any_page_conflict() {
    ...
    assert!(
        !merge.success,
        "generated-file conflict is still a conflict"
    );
...
fn remote_delete_of_locally_edited_page_keeps_local_content() {
    ...
    sb.git(&sb.theirs, &["commit", "-m", "Delete page"])
        .expect_success("their commit");
    sb.git(&sb.theirs, &["push", "origin", "main"])
        .expect_success("their push");
    sb.commit_change(
        &sb.ours,
    ...
    assert!(
        !merge.success,
        "modify/delete must conflict, not silently delete"
    );
    ...
    sb.git(&sb.ours, &["merge", "--abort"])
        .expect_success("abort");
    assert!(
        sb.ours.join(PAGE).exists(),
        "local file still present after abort"
    );
}
